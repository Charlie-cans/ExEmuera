...
namespace MinorShift.Emuera.GameProc
{
	...
	internal sealed class CalledFunction
	{
		...
		/// <summary>
		/// 1803beta005 预先匹配参数数量并代入默认值
        /// 1806+v6.99 修复当表达式中函数的参数传入了无效的#DIM变量时出现异常的问题
		/// 1808beta009 支持REF类型
		/// </summary>
		public UserDefinedFunctionArgument ConvertArg(IOperandTerm[] srcArgs, out string errMes)
		{
			...
			for (int i = 0; i < func.Arg.Length; i++)
			{
				...
				if (destArg.Identifier.IsReference)//引用传递的情况
				{
					if (term == null)
					{
					...
					//TODO 1810alpha007 想确定是否允许角色类型 目前倾向于不允许
					//类型检查
				...
				else if (term == null)//参数被省略时
				{
					term = func.Def[i];//代入默认值
					//1808beta001 没有默认值时设为错误
					//暂时放过
