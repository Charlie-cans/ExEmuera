...
namespace MinorShift.Emuera.GameData.Expression
{
	internal enum OperatorCode
	{
		...
		//一元 > "*/%" > "+-" > 比较                > 位运算     > 逻辑运算 > 赋值
		//xx   > 90    > 80   > 65,60(不等价优先)   > 50          > 40       > xx
		//优先级遵从原版
		NULL = 0,
		__PRIORITY_MASK__ = 0xFF,
		__UNARY__ = 0x10000,//一元
		__BINARY__ =  0x20000,//二元
        __TERNARY__ = 0x40000,//三元
		__UNARY_AFTER__ = 0x80000,//后置一元
		Plus = 0x0100 + 0x80 | __UNARY__ | __BINARY__,//"+"可用于一元运算
		Minus = 0x0200 + 0x80 | __UNARY__ | __BINARY__,//"-"可用于一元运算
		...
		BitXor = 0x1000 + 0x50 | __BINARY__,//"^"，优先级介于&和|之间。
		Not = 0x1100 | __UNARY__,//"!"一元
		BitNot = 0x1200 | __UNARY__,//"~"一元
		RightShift = 0x1300 + 0x70 | __BINARY__,//">>"
		LeftShift = 0x1400 + 0x70 | __BINARY__,//"<<"

        Ternary_a = 0x1800 + 0x05 | __TERNARY__,//"?"，三元运算符
        Ternary_b = 0x1900 + 0x10 | __TERNARY__,//"#"，三元运算符分隔符，因无法使用":"而用此代替
